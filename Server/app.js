...
function getTimes() {
    ...
    var timeLoop = current;
    timeString = "";
    for (var i = 0; i < 13; i++) {
        var tmp = timeLoop.getHours(), tag;
        if (tmp > 12) {
            tag = "PM";
            tmp -= 12;
        } else
            tag = "AM";
        var minute = timeLoop.getMinutes();
        if (minute == "0")
           minute += "0";
        timeString += tmp + ":" + minute + tag + " ";
        timeLoop = new Date(timeLoop.getTime() + -2*3600000);
    }

    var dateLoop = current;
    dateString = "";
    for (var j = 0; j < 10; j++) {
        dateString += dateLoop.getMonth() + 1 + '.' + dateLoop.getDate() + " ";
