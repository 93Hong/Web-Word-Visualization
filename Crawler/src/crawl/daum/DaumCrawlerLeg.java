...
public class DaumCrawlerLeg {
	...
	private String pre = "c:";//"/usr"; // "/usr"
	DaumCrawler da = new DaumCrawler();
	...
	public boolean crawl(String url, int File_Name) {
		try {
			...
			if (this.htmlDocument != null) {
				if (this.htmlDocument.body() != null) {
					if (this.htmlDocument.body().getElementsByAttributeValueMatching("id", "harmonyContainer") != null)
						article = this.htmlDocument.body().getElementsByAttributeValueMatching("id", "harmonyContainer").text();
					else
						return false;
				} else
					return false;
			...
			if (article != null && !article.isEmpty()) {
				String articleTime = this.links.get(0).split("v/")[1].substring(0, 12);
				String thirtyMinAgo = dateFormat.format(calendar.getTime());
				
				if (thirtyMinAgo.compareTo(articleTime) < 0) {
					//System.out.println(url + " : " + this.htmlDocument.title() + "\n");
					makeFile(File_Name);
	...
	public void makeFile(int fileName) {
		...
		try {
			String article = this.htmlDocument.body().getElementsByAttributeValueMatching("id", "harmonyContainer").text();

			if (!article.isEmpty()) {
				String file = pre + "/data/crawl/article/daum/" + fileName + ".txt";
